...
public class StageEditor : MonoBehaviour
{
    ...
    private void RefreshMouseMove() private bool PartOfSelection(Vector2Int Position, Stage_Data Data)
    {
        if (!Save_Data_Manager.Current_Settings.SelectionMode)
        {
            return true;
        }
        ...
        if (Data.Get_Tile(Position) == Save_Data_Manager.Current_Settings.Item_History[0])
        {
            return true;
        }
        ...
        if (CurrentAction == EditorAction.TouchBoxFill)
        {
            ...
            else if (Input.touchCount > 0)
            {
                ...
                for (int x = min.x; x <= max.x; x++)
                {
                    ...
                    {
                        if (Save_Data_Manager.Current_Settings.EraseMode)
                        {
                        ...
                        else
                        {
                            if (PartOfSelection(new Vector2Int(x, y), Preview_Data))
                            {
                                Preview_Data.Set_Tile(new Vector2Int(x, y),
                                    Save_Data_Manager.Current_Settings.Item_History[0]);
                            }
                        }
                    }
                }
    ...
    {
        if (!Save_Data_Manager.Current_Settings.BoxFillMode && Input.GetMouseButtonDown(0) &&
        ...
        else if (Save_Data_Manager.Current_Settings.BoxFillMode && Input.GetMouseButtonDown(0) &&
                 !OverUI(Input.mousePosition) &&
                 Current_Data.Tile_Deletable(SG.WorldToGrid(Camera.main.ScreenToWorldPoint(Input.mousePosition))))
        {
        ...
        else if (!Save_Data_Manager.Current_Settings.BoxFillMode && Input.touchCount == 1 &&
    ...
    {
        Save_Data_Manager.Current_Settings.Item_History =
            Save_Data_Manager.Current_Settings.Item_History.Distinct().ToList();
        Save_Data_Manager.Current_Settings.Item_History.Remove(NewItem);
        Save_Data_Manager.Current_Settings.Item_History.Insert(0, NewItem);
    }
    ...
    {
        if (GUIHelper.Raycast(Point) == null)
        {
            return false;
        }
        else
